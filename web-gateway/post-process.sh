...
intermediate_path=$output_webgw_path.tmp
cp -a "$input_webgw_path" "$intermediate_path"
...
case $host_os in
  macos|ios)
    case $host_os in
      macos)
        "$CODESIGN" -f -s "$MACOS_CERTID" -i "re.frida.WebGateway" "$intermediate_path" || exit 1
        ;;
      ios)
...
mv "$intermediate_path" "$output_webgw_path"
