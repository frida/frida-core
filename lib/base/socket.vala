namespace Frida {
	...
	public class WebService : Object {
		...
		private async SocketAddress do_start (Cancellable? cancellable) throws Error, IOError {
			...
			while (true) {
				...
				if (inet_address != null) {
					effective_address = listen_on_inet_address (main_server, inet_address);
				} else {
					try {
		...
		private void on_dynamic_interface_attached (string name, InetAddress ip) {
			...
			try {
				listen_on_inet_address (server, new InetSocketAddress (ip, endpoint_params.port));
			} catch (Error e) {
				return;
			}
		...
		private InetSocketAddress listen_on_inet_address (Soup.Server server, InetSocketAddress address) throws Error {
