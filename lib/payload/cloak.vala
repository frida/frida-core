namespace Frida {
	...
	public sealed class ThreadCountCloaker : Object {
		...
		private GetOsThreadStatQuick * get_os_thread_stat_quick_slot;
		private static GetOsThreadStatQuick old_get_os_thread_stat_quick_impl;
		...
		[CCode (has_target = false)]
		private delegate string GetOsThreadStatQuick (Posix.pid_t tid);
		...
		construct {
			...
			if (art != null) {
				art.enumerate_imports (imp => {
					if (imp.name == "_ZN3art15GetOsThreadStatEiPcm") {
						get_os_thread_stat_slot = (GetOsThreadStat *) imp.slot;
					}
					if (imp.name == "_ZN3art20GetOsThreadStatQuickEi") {
						get_os_thread_stat_quick_slot = (GetOsThreadStatQuick *) imp.slot;
					}
					if (imp.name == "read") {
				...
				if (get_os_thread_stat_slot != null) {
					old_get_os_thread_stat_impl = (GetOsThreadStat) update_slot((void **) get_os_thread_stat_slot, (void *) on_get_os_thread_stat);
				}
				if (get_os_thread_stat_quick_slot != null) {
					old_get_os_thread_stat_quick_impl = (GetOsThreadStatQuick) update_slot((void **) get_os_thread_stat_quick_slot, (void *) on_get_os_thread_stat_quick);
				}
				if (read_slot != null) {
		...
		public override void dispose () {
			...
			if (get_os_thread_stat_quick_slot != null) {
				update_slot ((void **) get_os_thread_stat_quick_slot, (void *) old_get_os_thread_stat_quick_impl);
			}
			if (read_slot != null) {
		...
		private static string on_get_os_thread_stat_quick (Posix.pid_t tid) {
			return old_get_os_thread_stat_quick_impl (tid);
		}
