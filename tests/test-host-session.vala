	...
	namespace Linux {
		...
		private static async void spawn (Harness h) {
			...
			try {
				...
				pid = yield host_session.spawn (Frida.Test.Labrats.path_to_executable ("sleeper"), HostSpawnOptions ());
	...
	namespace Darwin {
		...
		private static async void run_spawn_scenario_with_stdio (Harness h, string target_name) {
			...
			try {
				...
				pid = yield host_session.spawn (Frida.Test.Labrats.path_to_file (target_name), HostSpawnOptions ());
	...
	namespace Unix {
		...
		public static async void run_fork_scenario (Harness h, string target_path) {
			try {
				...
				var parent_pid = yield device.spawn (target_path);
