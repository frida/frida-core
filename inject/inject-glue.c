...
frida_inject_environment_init (void)
{
  ...
  g_assertion_set_handler (frida_inject_on_assert_failure, NULL);
  g_log_set_default_handler (frida_inject_on_log_message, NULL);
...
static void
frida_inject_on_assert_failure (const gchar * log_domain, const gchar * file, gint line, const gchar * func, const gchar * message, gpointer user_data)
{
  gchar * full_message;
  ...
  frida_inject_on_log_message (log_domain, G_LOG_LEVEL_ERROR, full_message, user_data);
  g_free (full_message);
...
static void
frida_inject_on_log_message (const gchar * log_domain, GLogLevelFlags log_level, const gchar * message, gpointer user_data)
{
#if defined (HAVE_DARWIN)
