...
pub fn ask_for_a_thread(code: u64, stack: u64, argument: u64) -> bool {
    ...
    put(crate::xnu_relay::THREAD_ANSWER, crate::xnu_relay::NOTHING_WANTED);
    put(crate::xnu_relay::THREAD_STACK, stack);
    put(crate::xnu_relay::THREAD_ARGUMENT, argument);
    put(crate::xnu_relay::THREAD_WANTED, code);

    loop {
        let said = unsafe {
            ((arena + crate::xnu_relay::THREAD_ANSWER) as *const u64).read_volatile()
        };
        if said != crate::xnu_relay::NOTHING_WANTED {
            return said == crate::xnu_relay::THREAD_STARTED;
        }
        crate::kernel::yield_now();
    }
}
