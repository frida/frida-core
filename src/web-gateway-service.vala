namespace Frida {
	public class WebGatewayService : Object {
		...
		private Soup.Server server = (Soup.Server) Object.new (typeof (Soup.Server));
		...
		private void on_websocket_opened (Soup.Server server, Soup.WebsocketConnection connection, string path,
				Soup.ClientContext client) {
			var peer = new Peer (connection, target_address);
		...
		private class Peer : Object {
			...
			private SocketConnection? target_connection;
			...
			private async void process_io () {
				try {
					...
					target_connection = yield client.connect_async (target_address, io_cancellable);

					maybe_process_pending_writes ();

					InputStream input = target_connection.input_stream;
					var buffer = new uint8[64 * 1024];
					while (true) {
			...
			private void maybe_process_pending_writes () {
				if (write_in_progress || pending_writes.is_empty || target_connection == null)
					return;
			...
			private async void process_pending_writes () {
				OutputStream output = target_connection.output_stream;
