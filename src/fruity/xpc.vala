	...
	public sealed class TunnelConnection : Object, AsyncInitable {
		...
		private async void create_netstack (InetAddress address) {
			try {
				...
				log_event ("_tunnel_netstack created with scope_id=%u\n", _tunnel_netstack.scope_id);
		...
		private void send_datagram (Bytes datagram) {
			stderr.printf ("\n");
			log_event ("send_datagram() connection=%p size=%zu\n", connection, datagram.get_size ());
			if (connection == null)
				return;
		...
		private bool on_socket_readable (DatagramBased datagram_based, IOCondition condition) {
			try {
				...
				log_event ("\treceived UDP packet, size: %zu\n", messages[0].bytes_received);
		...
		private void do_process_pending_writes () {
			...
			while (true) {
				...
				try {
					...
					log_event ("\tsent UDP packet, size: %zd\n", n);
				} catch (GLib.Error e) {
					continue;
				}
			}
		...
		private int on_recv_datagram (uint32 flags, uint8[] data) {
			log_event ("incoming_datagram() connection=%p size=%d\n", connection, data.length);
