namespace Zed.Service.WinjectorIpc {
	...
	public delegate void InjectFunc (uint32 target_pid, string filename_template) throws WinjectorError;

	public Variant? marshal_inject (Variant? arg, InjectFunc func) {
		uint32 target_pid;
		string filename_template;
		arg.get (INJECT_SIGNATURE, out target_pid, out filename_template);
		...
		try {
			func (target_pid, filename_template);
	...
	public async void invoke_inject (uint32 target_pid, string filename_template, WinIpc.Proxy proxy) throws WinjectorError {
		Variant response;

		try {
			response = yield proxy.query ("Inject", new Variant (INJECT_SIGNATURE, target_pid, filename_template), INJECT_RESPONSE);
