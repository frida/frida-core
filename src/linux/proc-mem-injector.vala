namespace Frida {
	private sealed class ProcMemInjectSession : Object {
		...
		private const uint DRAIN_MS = 200;
		...
		public static bool is_available (uint pid) {
#if X86 || X86_64 || ARM64
		...
		// The bootstrap stub lives inside malloc itself and the loader in a region it
		// mmap()s; see PTRACE-FREE-INJECTION.md for the design.
...
#if X86 || X86_64
			mem.write_memory (target, original[0:BOOTSTRAP_OFFSET]);
#elif ARM64
